...
docker run --rm \
  ...
  /bin/bash -c '
    ...
    bedgraph="$(find "$tmpdir" -type f \( -name "*.bedGraph" -o -name "*.bedgraph" -o -name "*.bg" \) -print -quit)"
    test -n "$bedgraph"
    test -s "$bedgraph"
    printf '\''GenMap BEDGraph: %s\n'\'' "$bedgraph"
    head -n 5 "$bedgraph"
    awk '\''BEGIN { OFS = "\t" } NR == 1 { print $1, $2, $3 }'\'' "$bedgraph" \
      | bedtools sort \
      | bedtools merge \
      > "$tmpdir/merged_interval.bed"
    test -s "$tmpdir/merged_interval.bed"
  '
