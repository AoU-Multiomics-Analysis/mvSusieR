...
`PrepareWindowGenotypes` extracts the requested dosage interval with `tabix` and writes the one-row dosage manifest. `PrepareWindowPhenotypes` runs `scripts/prepare_trans_window.R`; for now, each gzipped phenotype BED is read fully into R and filtered in memory. Expression and splicing files use the first three columns for chromosome/start/end and column 4 for the phenotype ID. By default, all trans-associated IDs in the supplied TensorQTL file and all cis phenotypes overlapping the selected window are retained; set `extract_cis_window_phenotypes` to `false` for trans-only preparation.

The outputs are `window_dosage.tsv`, a one-row `window_manifest.tsv` compatible with the existing dosage-manifest reader, per-modality files under `phenotype_subsets/`, `window_phenotypes.tsv` for the existing `prepare_window.R` interface, and `window_qc.tsv`. The phenotype subset files are intentionally not tabix-indexed in this first implementation; indexing or a one-time in-memory/columnar cache can be added later if full-file scans become the bottleneck.
